...
def load_stats(access_token: Optional[str] = None) -> None:
    ...
    stat_source = unified_sklik_stat_source(access_token)
    load_info = pipeline.run(stat_source)
    print(load_info)


def load_all(access_token: Optional[str] = None) -> None:
    pipeline = dlt.pipeline(
        pipeline_name="seznam_sklik_all",
        destination="duckdb",
        dataset_name="seznam_sklik",
    )
    ...
    setting_source = unified_sklik_source(access_token)
    stat_source = unified_sklik_stat_source(access_token)
    load_info = pipeline.run([setting_source, stat_source])
    print(load_info)
